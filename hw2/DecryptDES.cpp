...
string FinalPermutation(string text);
...
string DecryptDES(string key, string cipherText)
{
    string result = cipherText, subkey = PC(1, key);
    ...
    for (int i = 0; i < 16; i++)
    {
        string left = result.substr(0, 32), right = result.substr(32);
        ...
        case 9:
        case 16:
            subkey = subkey.substr(27, 28) + subkey.substr(0, 27) + subkey.substr(55) + subkey.substr(28, 55);
            break;
        default:
            subkey = subkey.substr(26, 28) + subkey.substr(0, 26) + subkey.substr(54) + subkey.substr(28, 54);
            break;
        }

        left = exclusiveOR(left, Feistel(PC(2, subkey), right));
...
string InitialPermutation(string text) 
{
    ...
    for (int i = 0; i < 64; i++)
    {
        result.push_back(text[ipVector[i] - 1]);
    }
    return result;
}
...
{
    string result;
    const int fpVector[64] = { 40, 8, 48, 16, 56, 24, 64, 32,
...
string PC(int type, string text)
{
    ...
    if (type == 1)
    {
        const int PC1Vector[56] = { 57, 49, 41, 33, 25, 17, 9,
                                    1, 58, 50, 42, 34, 26, 18,
                                    10, 2, 59, 51, 43, 35, 27,
                                    19, 11, 3, 60, 52, 44, 36,
    ...
    {
        const int PC2Vector[48] = { 14, 17, 11, 24, 1, 5,
                                    3, 28, 15, 6, 21, 10,
                                    23, 19, 12, 4, 26, 8,
                                    16, 7, 27, 20, 13, 2,
                                    41, 52, 31, 37, 47, 55,
...
string Feistel(string key, string text)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        result += Substitution(i + 1, text.substr(i * 6, i * 6 + 6));
    }
...
string Substitution(int digit, string text)
{
    ...
    return DecToBin(box[digit - 1][row][column]);
}
...
{
    string result;
    int PVector[32] = { 16, 7, 20, 21, 29, 12, 28, 17,
